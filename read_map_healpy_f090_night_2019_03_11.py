...
# path for figures
pathFig = "./figures/cmb_map/planck_act_coadd_2019_03_11/f090_prelim/"
...
hp.write_map(pathIn+"healpix_f090_prelim_map_mono.fits", hMap, overwrite=True)
hp.write_map(pathIn+"healpix_f090_prelim_div_mono.fits", hHitMap, overwrite=True)
...
fig.savefig(pathFig+"f090_hit_0.5max.pdf")
fig.clf()
...
fig.savefig(pathFig+"f090_hit_log.pdf")
fig.clf()
...
fig.savefig(pathFig+"f090_footmask.pdf")
fig.clf()

# save the footprint to file
hp.write_map(pathIn+"f090_footprint_mask.fits", footMask, overwrite=True)

# read the footprint mask
footMask = hp.read_map(pathIn+"f090_footprint_mask.fits")
...
fig.savefig(pathFig+"f090_mask_foot_planck.pdf")
fig.clf()
...
fig.savefig(pathFig+"f090_mask_foot_planck_smoothed.pdf")
fig.clf()
...
fig=plt.figure(0)
hp.mollview(fullMask, fig=0, title="Footprint", coord=None, cbar=True, unit='')
fig.savefig(pathFig+"f090_mask_foot_planck_thresh.pdf")
fig.clf()

# save the full mask
hp.write_map(pathIn+"f090_mask_foot_planck_ps.fits", fullMask, overwrite=True)

# read the full mask
fullMask = hp.read_map(pathIn+"f090_mask_foot_planck_ps.fits")
...
# save
enmap.write_map(pathIn+"f090_mask_foot_planck_ps_car.fits", carFullMask)
...
hp.mollview(logHit, fig=0, title="Hit count", min=np.min(logHit), max=np.max(logHit), coord=None, cbar=False, unit='')
fig.savefig(pathFig+"f090_masked_hit_log.pdf")
fig.clf()

# save this map to file, for later illustrations
hp.write_map(pathIn+"f090_masked_hit_log.fits", logHit, overwrite=True)
...
fig.savefig(pathFig+"f090_T_masked.pdf")
fig.clf()
...
fig.savefig(pathFig+"f090_Q_masked.pdf")
fig.clf()
...
fig.savefig(pathFig+"f090_U_masked.pdf")
fig.clf()
...
lCen, Cl, sCl = powerSpectrum(hMap[0], mask=fullMask, theory=[ftotal], fsCl=None, nBins=101, lRange=None, plot=True, path=pathFig+"f090_power_T_masked.pdf", save=True)
...
np.savetxt(pathIn+"f090_power_T_masked.txt", data)
...
ax.legend(loc=3)
#
fig.savefig(pathFig+"f090_power_ACTPlanck_vs_P143.pdf")
fig.clf()
